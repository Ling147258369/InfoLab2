...
def click1():
    ...
    entry_content1=str_to_hex(entry_content1)
    entry_content3=str_to_hex(entry_content3)
    print(int(entry_content1, 16))
    print(int(entry_content3, 16))
    s = my.encrypt(int(entry_content1,16),int(entry_content3,16))
    x=[0]*2
    for i in range(2):
       x[i]=s[2*i]*16+s[2*i+1]
    print(x)
    x1=x[0]
    x2=x[1]
    e4.insert(INSERT, chr(x1))
    e4.insert(INSERT, chr(x2))
def click2():

    entry_content2 = e2.get()
    entry_content3 = e3.get()
    e4.delete("1.0", tk.END)
    entry_content2 = str_to_hex(entry_content2)
    entry_content3 = str_to_hex(entry_content3)
    print(int(entry_content2,16))
    print(int(entry_content3, 16))
    s = my.decrypt(int(entry_content2, 16), int(entry_content3, 16))
    x = [0] * 2
    for i in range(2):
        x[i] = s[2 * i] * 16 + s[2 * i + 1]
    print(x)
...
if __name__=="__main__":
 ...
 lab1=Label(root,text="  请输入2位ASCII明文：").grid(row=0)
 lab2=Label(root,text="  请输入2位ASCII暗文：").grid(row=1)
 lab3=Label(root,text="  请输入2位ASCII密钥：").grid(row=2)
...
while k != 0:
    if k == 1:
        ...
        # l=0x64c0
        # l=my.slipt(l)
        # l=my.mixColumns(my.MIX_C,l)
        # print(l)
        ciphertext = 0b1100010000111001
        #ciphertext = 0xc439
